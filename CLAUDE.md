...
| `insights.py` | Per-sample digest from the history store + TaiCoL: phenophase months (花/果/苞 with counts, from iNat annotations only — sparse → 不足/尚無, never inferred; single-obs phases dropped from the headline as noise, raw counts kept in JSON), sampling depth (obs/年/觀察者/span), GPS proximity (≤50 m share), 特有/保育. Drives the unit-page insight line + transect table. | `data/registry/insights.json` |
